...
async def main():
    ...
    async with websockets.connect(WS_URL, max_size=10 * 1024 * 1024, proxy=None) as ws:
        while True:
            ...
            missing_warning_ids = data.get("missing_warning_ids", []) if last_ok else []

            if missing_warning_ids:
                print("WARNING missing tracks:", missing_warning_ids)
                print('\a', end='')
            ...
            for p in persons:
                ...
                vel = float(p.get("velocity", 0.0))

                color = color_for_state(state)

                if state in ("warning", "danger"):
                    print('\a', end='')  # hxhtiko
                ...
                # label
                if conf is not None:
                    label = f"ID {track_id} {state} v={vel:.1f} {conf:.2f}"
                else:
                    label = f"ID {track_id} {state} v={vel:.1f}"
